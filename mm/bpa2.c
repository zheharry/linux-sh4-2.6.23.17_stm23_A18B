...
#if 0
caddr_t  bigphysarea_alloc(unsigned int size)
{
	return 	bigphysarea_alloc_pages(PAGE_ALIGN(size) >> PAGE_SHIFT,1, GFP_KERNEL);
}

void  bigphysarea_free(caddr_t  addr,unsigned int size)
{
	return 	bigphysarea_free_pages(addr);
}

caddr_t  bigphysarea_alloc_pages(int count, int align, int priority)
{
	return 	__bigphysarea_alloc_pages(count, align, priority, NULL, 0);
}
